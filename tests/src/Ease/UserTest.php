<?php

declare(strict_types=1);

/**
 * This file is part of the EaseCore package.
 *
 * (c) Vítězslav Dvořák <info@vitexsoftware.cz>
 *
 * For the full copyright and license information, please view the LICENSE
 * file that was distributed with this source code.
 */

namespace Test\Ease;

use Ease\User;
use PHPUnit\Framework\Attributes\CoversClass;

/**
 * Generated by PHPUnit_SkeletonGenerator on 2016-01-17 at 23:58:22.
 */
#[CoversClass(User::class)]
class UserTest extends AnonymTest
{
    protected User $user;

    /**
     * Sets up the fixture, for example, opens a network connection.
     * This method is called before a test is executed.
     */
    protected function setUp(): void
    {
        parent::setUp();
        $this->user = new User();
    }

    /**
     * Tears down the fixture, for example, closes a network connection.
     * This method is called after a test is executed.
     */
    protected function tearDown(): void
    {
    }

    /**
     * Test Constructor.
     *
     * @covers \Ease\User::__construct
     */
    public function testConstructor(): void
    {
        $classname = \get_class($this->user);

        // Get mock, without the constructor being called
        $mock = $this->getMockBuilder($classname)
            ->disableOriginalConstructor()
            ->getMockForAbstractClass();
        $mock->__construct('test', ['debug' => false]);

        $this->assertEmpty($mock->getData());
    }

    /**
     * @covers \Ease\User::getUserName
     */
    public function testGetUserName(): void
    {
        $this->user->setDataValue('login', 'vitex');
        $this->assertEquals('vitex', $this->user->getUserName());
    }

    /**
     * @covers \Ease\Person::getUserEmail
     * @covers \Ease\User::getUserEmail
     */
    public function testGetUserEmail(): void
    {
        $this->user->setDataValue('email', 'info@vitexsoftware.cz');
        $this->assertEquals('info@vitexsoftware.cz', $this->user->getUserEmail());
    }

    /**
     * @covers \Ease\User::draw
     *
     * @param null|mixed $whatWant
     */
    public function testDraw($whatWant = null): void
    {
        $this->assertEquals('<img class="avatar" src="data:image/svg+xml;base64,">', $this->user->draw());
    }

    /**
     * @covers \Ease\User::getIcon
     */
    public function testGetIcon(): void
    {
        $this->assertEquals('data:image/svg+xml;base64,', $this->user->getIcon());
        $this->user->setDataValue('email', 'info@vitexsoftware.cz');
        $this->assertEquals('http://www.gravatar.com/avatar/f00226a1c6fe891010a2543531ed432c?s=800&d=mm&r=g', $this->user->getIcon());
    }

    /**
     * @covers \Ease\User::tryToLogin
     */
    public function testTryToLogin(): void
    {
        $this->assertEmpty($this->user->tryToLogin([]));
        $this->assertEmpty($this->user->tryToLogin(['login' => 'noone']));
        $this->assertEmpty($this->user->tryToLogin(['login' => '', 'password' => '']));
        $this->assertFalse($this->user->logged, $this->user->tryToLogin(['login' => 'vitex', 'password' => 'secret']));
    }

    /**
     * @covers \Ease\User::isAccountEnabled
     */
    public function testIsAccountEnabled(): void
    {
        $this->assertTrue($this->user->isAccountEnabled());
        $this->user->disableColumn = 'state';
        $this->user->setDataValue('state', true);
        $this->assertFalse($this->user->isAccountEnabled());
        $this->user->setDataValue('state', false);
        $this->assertTrue($this->user->isAccountEnabled());
    }

    /**
     * @covers \Ease\User::loginSuccess
     */
    public function testLoginSuccess(): void
    {
        $this->user->loginSuccess();
        $this->assertTrue($this->user->logged);
    }

    /**
     * @covers \Ease\User::authentize
     */
    public function testAuthentize(): void
    {
        $this->assertFalse($this->user->authentize('pass'));
    }

    /**
     * @covers \Ease\User::validatePassword
     */
    public function testValidatePassword(): void
    {
        $this->user->setDataValue('password', '240885b263943a74849aa0b2d3b1285b:2d');
        $this->assertTrue($this->user->validatePassword('password'));
    }

    /**
     * @covers \Ease\User::encryptPassword
     */
    public function testEncryptPassword(): void
    {
        $this->assertNotEmpty(\Ease\User::encryptPassword('password'));
    }

    /**
     * @covers \Ease\User::passwordValidation
     */
    public function testPasswordValidation(): void
    {
        $this->assertFalse(\Ease\User::passwordValidation('heslo', '813c98b71749197010bb458facd84021'));
        $this->assertTrue(\Ease\User::passwordValidation('password', '240885b263943a74849aa0b2d3b1285b:2d'));
    }

    /**
     * @covers \Ease\User::encryptPassword
     * @covers \Ease\User::passwordValidation
     */
    public function testPassword(): void
    {
        $encrypted = \Ease\User::encryptPassword('password');
        $this->assertNotEmpty($encrypted);
        $this->assertTrue(\Ease\User::passwordValidation('password', $encrypted));

        $this->assertFalse(\Ease\User::passwordValidation('X', $encrypted));

        $this->assertFalse(\Ease\User::passwordValidation('X', 'x:x:x'));
    }

    /**
     * @covers \Ease\User::passwordChange
     */
    public function testPasswordChange(): void
    {
        $this->assertFalse($this->user->passwordChange('newpass'));
    }

    /**
     * @covers \Ease\User::getUserID
     */
    public function testGetUserID(): void
    {
        $this->assertEquals(0, $this->user->getUserID());
    }

    /**
     * @covers \Ease\User::getUserLogin
     * @covers \Ease\User::setUserLogin
     */
    public function testUserLogin(): void
    {
        $this->assertTrue($this->user->setUserLogin('vitex'));
        $this->assertEquals('vitex', $this->user->getUserLogin());
    }

    /**
     * @covers \Ease\User::getPermission
     */
    public function testGetPermission(): void
    {
        $this->assertEquals('', $this->user->getPermission());
    }

    /**
     * @covers \Ease\User::logout
     */
    public function testLogout(): void
    {
        $this->assertTrue($this->user->logout());
        $this->assertFalse($this->user->logged);
    }

    /**
     * @a
     *
     * @covers \Ease\User::getSettingValue
     */
    public function testGetSettingValue(): void
    {
        $this->user->setSettingValue('test', 'tested');
        $this->assertEquals('tested', $this->user->getSettingValue('test'));
    }

    /**
     * @covers \Ease\User::setSettings
     */
    public function testSetSettings(): void
    {
        $this->user->setSettings(['key' => 'value']);
        $this->assertEquals('value', $this->user->getSettingValue('key'));
    }

    /**
     * @covers \Ease\User::setSettingValue
     */
    public function testSetSettingValue(): void
    {
        $this->user->setSettingValue('test', 'tested');
        $this->assertEquals('tested', $this->user->getSettingValue('test'));
    }

    /**
     * @covers \Ease\User::loadPermissions
     */
    public function testLoadPermissions(): void
    {
        $this->assertEquals('', $this->user->loadPermissions());
    }

    /**
     * @covers \Ease\User::getName
     */
    public function testGetName(): void
    {
        $this->assertEquals('Ease\User:@127.0.0.1 [tester]', $this->user->getName());
    }

    /**
     * @covers \Ease\User::getGravatar
     */
    public function testGetGravatar(): void
    {
        $this->assertEquals('http://www.gravatar.com/avatar/f00226a1c6fe891010a2543531ed432c?s=80&d=mm&r=g', $this->user->getGravatar('info@vitexsoftware.cz'));
    }

    /**
     * @covers \Ease\User::setObjectName
     */
    public function testSetObjectName(): void
    {
        $this->assertEquals('Ease\User:@127.0.0.1 [tester]', $this->user->setObjectName());
    }
}
