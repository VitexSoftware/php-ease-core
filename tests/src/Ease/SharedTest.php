...
class SharedTest extends AtomTest
{
    ...
    {
        global $_SESSION;
        $classname = \get_class($this->shared);
    ...
    {
        $this->shared->setConfigValue('test', true);
        $this->assertTrue($this->shared->getConfigValue('test'));
    }
    ...
    public function testLoadConfig(): void
    {
        ...
        $this->shared->loadConfig($env, true);
        $this->assertEquals(['KEY' => 'VALUE', 'FOO' => 'BAR', 'debug' => 'true', 'test' => true], $this->shared->configuration);
        $this->shared->loadConfig($json, true);
        $this->assertArrayHasKey('opt', $this->shared->configuration);
        $this->assertTrue(\defined('KEY'));
        $this->assertEquals('optvalue', $this->shared->getConfigValue('opt'));
        $this->assertEquals('keyvalue', $this->shared->getConfigValue('KEY'));
        $this->expectException('Exception');
        $this->shared->loadConfig('tests/Bootstrap.php', true);
    }
    ...
    {
        $this->assertIsInt($this->shared->saveStatusMessages());
    }
    ...
    {
        $this->assertIsArray($this->shared->loadStatusMessages());
    }
